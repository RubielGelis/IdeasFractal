IF OBJECT_ID('dbo.fnza_jsonaxml', 'P') IS NOT NULL
    DROP FUNCTION dbo.fnza_jsonaxml;
GO
CREATE FUNCTION dbo.fnza_jsonaxml(
    @json    varchar(max)
)
RETURNS xml
AS
...
BEGIN;
    DECLARE @output varchar(max), @key varchar(max), @value varchar(max),
        ...
        @tab char(1)=CHAR(9), @cr char(1)=CHAR(13), @lf char(1)=CHAR(10),@keyAux VARCHAR(MAX);

    --- Clean up the JSON syntax by removing line breaks and tabs and
    --- trimming the results of leading and trailing spaces:
    SET @json=LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(@json, @cr, ''), @lf, ''), @tab, ''),SPACE(2),'')));
	SET @json=RTRIM(@json)
    ...
    WHILE (@json!='') BEGIN;
        ...
        WHILE (@array IS NOT NULL) BEGIN;
            ...
            IF (@value IS NULL AND LEFT(@json, 1)='{') BEGIN;
                ...
                END;
                SET @value=CAST(
                    dbo.fnza_jsonaxml(LEFT(@json, @offset))
                        AS varchar(max));
            ...
            END;
			SET @KeyAux = CASE WHEN RIGHT(@key,2)='es' AND RIGHT(@key,3)<>'ces' THEN LEFT(@key,LEN(@key)-2)
							   WHEN RIGHT(@key,1)='s' THEN LEFT(@key,LEN(@key)-1)	
						  END
            --- Append @key and @value to @output:
            SET @output=@output+@lf+@cr+
                REPLICATE(@tab, @@NESTLEVEL-1)+
                '<'+@key+'>'+
					CASE WHEN @array=1 THEN case when isnull(@KeyAux,'')<> '' then '<'+@KeyAux+'>' else '' end ELSE '' END+
						ISNULL(REPLACE(
							REPLACE(@value, '\"', '"'), '\\', '\'),'')+
						(CASE WHEN @nested=1
					...
					CASE WHEN @array=1 THEN case when isnull(@KeyAux,'')<> '' then '</'+@KeyAux+'>' else '' end ELSE '' END+
                '</'+@key+'>';
			SET @output = REPLACE(@output,'</'+@key+'>'++@lf+@cr+REPLICATE(@tab, @@NESTLEVEL-1)+'<'+@key+'>','');
            --- And again, error checks:
            ---
            --- 1. If these are multiple values, the next character
            ---    should be a comma:
            IF (@array=0 AND @json!='' AND LEFT(@json, 1)!=',')
                RETURN @output+@json+' '+'Expected "," after value, found "'+
                    LEFT(@json, 1)+'"!';
    ...
    END;
    --- Return the output:
    RETURN CONVERT(XML,@output)--CAST(@output AS xml);
END;
GO
